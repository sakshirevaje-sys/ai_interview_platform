...
export function calculateRiskScore(violationTypes) {
  ...
  const volumeScore = Math.min(60, Math.round((violationTypes.length / MAX_VIOLATIONS) * 60));
  const severityScore = Math.min(40, Math.round((totalWeight / (violationTypes.length * 5)) * 40));
